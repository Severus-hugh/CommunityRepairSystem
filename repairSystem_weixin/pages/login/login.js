...
Page({
  data: {
    avatarUrl: '../../images/weixin.png', // 未登录时的状态图片
    userInfo: {}, // 存储用户信息列表
    hasUserInfo: false,
    logged: false,
    takeSession: false,
  ...
  getUI(e) { //点击授权登录时产生的监听事件
    ...
    wx.getUserProfile({ //获取用户信息。页面产生点击事件（例如 button 上 bindtap 的回调中）后才可调用，每次请求都会弹出授权窗口，用户同意后返回 userInfo,用于替换 wx.getUserInfo
      ...
      success(res) { //接口调用成功的回调函数
        console.log('获取', res)
        console.log('输出userInfo中的avatarUrl：');
        ...
        wx.navigateTo({//跳转到非Tabbar页面
          url:"/pages/logins/logins"//此处跳转到身份选择页面
        })
        ...
        wx.showToast({ //显示消息提示框
          image: "../../images/right.png", //自定义图标的本地路径，image 的优先级高于 icon
